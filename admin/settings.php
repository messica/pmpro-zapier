...
function pmproz_settings_general() {
	?>
	<p>
		<?php _e('Enter these when connecting to a Paid Memberships Pro account in Zapier.', 'pmproz'); ?>
	</p>
	<table class="form-table">
		<tr>
			<th scope="row"><?php _e('Webhook Handler', 'pmproz'); ?></th>
			<td>
				<input type="text" readonly size=100 value="<?php echo plugins_url('pmpro-zapier/includes/webhook-handler.php'); ?>">
			</td>
		</tr>
	</table>
	<?php
...
function pmproz_settings_field_pmpro_added_order() {
	...
	<label for="pmpro_added_order">
		...
		<?php _e('Update Zapier when a new order is added.', 'pmproz'); ?>
	</label>
	<?php
}
...
function pmproz_settings_field_pmpro_updated_order() {
	...
	<label for="pmpro_updated_order">
		...
		<?php _e('Update Zapier when an order is updated.', 'pmproz'); ?>
	</label>
	<?php
}
...
function pmproz_settings_field_pmpro_after_change_membership_level() {
	...
	<label for="pmpro_after_change_membership_level">
		...
		<?php _e('Update Zapier when a user changes membership levels. The old level status will be added if available.', 'pmproz'); ?>
	</label>
	<?php
}
...
<div class="wrap">
	<form action="options.php" method="POST">
		...
		<p>
			...
			For more information, visit ', 'pmproz') . '<a href=http://zapier.com>Zapier.com</a>'; ?>
		</p>
