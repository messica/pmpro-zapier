...
/**
 * Function to Register the Query Var for Zapier Trigger.
 * @param $vars Array of query vars.
 * @return array.
 */
function pmproz_add_query_trigger( $vars ) {
	$vars[] = 'pmproz_zapier_trigger';

	return $vars;
}
add_filter( 'query_vars', 'pmproz_add_query_trigger' );

/**
 * Function to handle webhook calls received from Zapier.
 */
function pmproz_webhook_handler_action() {

	if( 1 == intval( get_query_var( 'pmproz_zapier_trigger' ) ) ) {
...
add_action( 'template_redirect', 'pmproz_webhook_handler_action' );
